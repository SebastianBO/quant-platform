...
export default async function FinancingCashFlowPage({ params }: Props) {
  ...
  const schemas = [
    ...
    getTableSchema({
      name: `${symbol} Financing Cash Flow History`,
      description: `Historical Financing Cash Flow data for ${companyName} (${symbol})`,
      url: pageUrl,
      columns: ['Period', 'Financing Cash Flow', 'Change'],
      rowCount: 5,
    }),
  ]
