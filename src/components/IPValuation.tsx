    ...
    <Card className="w-full">
      ...
      <CardContent>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
          <div className="bg-gradient-to-br from-emerald-500/20 to-emerald-500/5 p-4 rounded-lg text-center border border-emerald-500/30">
            <p className="text-muted-foreground text-sm">Total IP Value</p>
            <p className="text-2xl font-bold text-emerald-500">{formatCurrency(totalIPValue)}</p>
          </div>
          <div className="bg-secondary/50 p-4 rounded-lg text-center">
            <p className="text-muted-foreground text-sm">IP as % of Market Cap</p>
            <p className={`text-2xl font-bold ${ipAsMarketCapPct > 30 ? 'text-emerald-500' : ipAsMarketCapPct > 15 ? 'text-yellow-500' : 'text-muted-foreground'}`}>
          ...
          <div className="bg-secondary/50 p-4 rounded-lg text-center">
            <p className="text-muted-foreground text-sm">IP Assets Found</p>
            <p className="text-2xl font-bold">{ipAssets.length}</p>
          </div>
        </div>

        {/* Interpretation */}
        <div className={`p-4 rounded-lg mb-6 ${ipAsMarketCapPct > 30 ? 'bg-emerald-500/10 border border-emerald-500/30' : ipAsMarketCapPct > 15 ? 'bg-yellow-500/10 border border-yellow-500/30' : 'bg-secondary/30 border border-border'}`}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
          ...
          <div>
            <p className="text-sm text-muted-foreground mb-2">IP Value by Segment</p>
            <div className="h-64">
            ...
            <div className="h-64">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={ipAssets} layout="vertical">
                  ...
                  <YAxis type="category" dataKey="segment" stroke="hsl(var(--muted-foreground))" width={100} tick={{ fontSize: 11 }} />
                  <Tooltip
                    contentStyle={{ backgroundColor: 'hsl(var(--card))', border: '1px solid hsl(var(--border))' }}
                    formatter={(value: number) => [formatCurrency(value), 'IP Value']}
                  />
        ...
        <div className="overflow-x-auto">
          <p className="text-sm text-muted-foreground mb-2">Discovered IP Assets</p>
                ...
                <tr key={i} className="border-b border-border/50">
                  ...
                  <td className="p-3">
                    <span className={`px-2 py-1 rounded text-xs ${
                      asset.type === 'patent' ? 'bg-blue-500/20 text-blue-500' :
                  ...
                  <td className="p-3 text-right font-bold text-emerald-500">{formatCurrency(asset.value)}</td>
                </tr>
              ))}
            </tbody>
            <tfoot>
              <tr className="bg-secondary/30">
                ...
                <td className="p-3 text-right font-bold text-emerald-500">{formatCurrency(totalIPValue)}</td>
              </tr>
            </tfoot>
          </table>
        </div>
        ...
        <div className="mt-6 p-4 bg-secondary/30 rounded-lg text-sm text-muted-foreground">
          <p className="font-medium text-foreground mb-1">Valuation Methodology: Relief from Royalty</p>
