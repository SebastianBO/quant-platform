  ...
  if (!dcfResults) {
    return (
      <Card className="w-full">
        <CardHeader>
          <CardTitle>DCF Valuation - {ticker}</CardTitle>
        </CardHeader>
        <CardContent>
          <p className="text-muted-foreground">Insufficient data for DCF analysis. Requires positive free cash flow and shares outstanding.</p>
        </CardContent>
      </Card>
    )
  }
  ...
  return (
    <Card className="w-full">
      <CardHeader>
        ...
        <p className="text-sm text-muted-foreground">Institutional-Grade Discounted Cash Flow Analysis</p>
      </CardHeader>
      <CardContent className="space-y-8">
        ...
        <div className="bg-secondary/50 p-4 rounded-lg border border-border">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-4">
            <div>
              <label className="text-xs text-muted-foreground">Risk-Free Rate (10Y Treasury)</label>
              <input
                type="range"
                min="2"
                max="7"
              ...
              />
              <span className="text-blue-500 font-mono text-sm">{riskFreeRate.toFixed(2)}%</span>
            </div>
            <div>
              <label className="text-xs text-muted-foreground">Equity Risk Premium</label>
              <input
                type="range"
                min="3"
                max="8"
                ...
                className="w-full accent-blue-500"
              />
              <span className="text-blue-500 font-mono text-sm">{equityRiskPremium.toFixed(2)}%</span>
            </div>
            <div>
              <label className="text-xs text-muted-foreground">Beta (Market Risk)</label>
              <span className="block text-blue-500 font-mono text-sm">{beta.toFixed(2)}</span>
            </div>
            <div>
              <label className="text-xs text-muted-foreground">D/E Ratio</label>
              <span className="block text-blue-500 font-mono text-sm">{debtToEquity.toFixed(2)}</span>
            </div>
          </div>
          <div className="grid grid-cols-3 md:grid-cols-6 gap-2 text-center text-sm">
            <div className="bg-secondary/50 p-2 rounded">
              ...
              <p className="text-blue-500 font-mono">{waccCalculation.costOfEquity.toFixed(2)}%</p>
            </div>
            <div className="bg-secondary/50 p-2 rounded">
              <p className="text-muted-foreground text-xs">Cost of Debt</p>
              <p className="text-blue-500 font-mono">{waccCalculation.costOfDebt.toFixed(2)}%</p>
            </div>
            <div className="bg-secondary/50 p-2 rounded">
              <p className="text-muted-foreground text-xs">Equity Weight</p>
              <p className="text-blue-500 font-mono">{(waccCalculation.equityWeight * 100).toFixed(1)}%</p>
            </div>
            <div className="bg-secondary/50 p-2 rounded">
              <p className="text-muted-foreground text-xs">Debt Weight</p>
              <p className="text-blue-500 font-mono">{(waccCalculation.debtWeight * 100).toFixed(1)}%</p>
            </div>
            <div className="bg-secondary/50 p-2 rounded">
              <p className="text-muted-foreground text-xs">Tax Rate</p>
              <p className="text-blue-500 font-mono">{(waccCalculation.taxRate * 100).toFixed(0)}%</p>
            </div>
            <div className="bg-emerald-500/20 p-2 rounded border border-emerald-500/50">
              ...
              <p className="text-emerald-500 font-mono font-bold">{waccCalculation.wacc.toFixed(2)}%</p>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <div>
            <label className="text-sm text-muted-foreground">Growth Rate (Year 1)</label>
            <input
              type="range"
              min="0"
              max="35"
            ...
            />
            <span className="text-emerald-500 font-mono">{growthRate.toFixed(0)}%</span>
          </div>
          <div>
            <label className="text-sm text-muted-foreground">Terminal Growth</label>
            <input
              type="range"
              min="1"
              max="4"
              ...
              className="w-full accent-emerald-500"
            />
            <span className="text-emerald-500 font-mono">{terminalGrowth.toFixed(2)}%</span>
          </div>
          <div>
            ...
            <input
              ...
              className="w-full accent-emerald-500"
            />
            <span className="text-emerald-500 font-mono">{years} years</span>
          </div>
          <div>
            <label className="text-sm text-muted-foreground">Margin of Safety</label>
            <input
              type="range"
              min="0"
              max="50"
            ...
            />
            <span className="text-yellow-500 font-mono">{marginOfSafety}%</span>
          </div>
        </div>

        {/* Main Results */}
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <div className="bg-secondary/50 p-4 rounded-lg text-center border border-border">
          ...
          <div className="bg-secondary/50 p-4 rounded-lg text-center border border-border">
            <p className="text-muted-foreground text-sm">DCF Fair Value</p>
            <p className={`text-2xl font-bold font-mono ${dcfResults.fairValuePerShare > currentPrice ? 'text-emerald-500' : 'text-red-500'}`}>
              ${dcfResults.fairValuePerShare.toFixed(2)}
            </p>
          </div>
          <div className="bg-secondary/50 p-4 rounded-lg text-center border border-border">
            <p className="text-muted-foreground text-sm">With {marginOfSafety}% MOS</p>
            <p className={`text-2xl font-bold font-mono ${dcfResults.withMarginOfSafety > currentPrice ? 'text-yellow-500' : 'text-red-500'}`}>
              ${dcfResults.withMarginOfSafety.toFixed(2)}
            </p>
          </div>
          <div className="bg-secondary/50 p-4 rounded-lg text-center border border-border">
            <p className="text-muted-foreground text-sm">Upside/Downside</p>
            <p className={`text-2xl font-bold font-mono ${dcfResults.upside > 0 ? 'text-emerald-500' : 'text-red-500'}`}>
        ...
        <div className="bg-secondary/50 p-4 rounded-lg border border-border">
          <h3 className="text-lg font-bold mb-4">Enterprise Value Breakdown</h3>
          <div className="grid grid-cols-2 md:grid-cols-5 gap-4 text-center text-sm">
            <div className="bg-secondary/50 p-3 rounded">
              <p className="text-muted-foreground text-xs">PV of FCF</p>
              <p className="text-emerald-500 font-mono font-bold">{formatNumber(dcfResults.pvOfFCF)}</p>
            ...
            <div className="bg-secondary/50 p-3 rounded">
              ...
              <p className="text-blue-500 font-mono font-bold">{formatNumber(dcfResults.terminalPV)}</p>
            ...
            <div className="bg-secondary/50 p-3 rounded">
              ...
              <p className="text-foreground font-mono font-bold">{formatNumber(dcfResults.enterpriseValue)}</p>
            </div>
            <div className="bg-secondary/50 p-3 rounded">
              <p className="text-muted-foreground text-xs">Net Debt</p>
              <p className={`font-mono font-bold ${dcfResults.netDebt > 0 ? 'text-red-500' : 'text-emerald-500'}`}>
                {dcfResults.netDebt > 0 ? '-' : '+'}{formatNumber(Math.abs(dcfResults.netDebt))}
              </p>
            </div>
            <div className="bg-emerald-500/20 p-3 rounded border border-emerald-500/50">
              <p className="text-foreground text-xs font-bold">Equity Value</p>
              <p className="text-emerald-500 font-mono font-bold">{formatNumber(dcfResults.equityValue)}</p>
            </div>
          </div>
        </div>
        ...
        {monteCarloResults && (
          <div className="bg-secondary/50 p-4 rounded-lg border border-border">
            ...
            <div className="grid grid-cols-4 md:grid-cols-7 gap-2 text-center text-sm mb-4">
              <div className="bg-secondary/50 p-2 rounded">
                <p className="text-muted-foreground text-xs">10th %ile</p>
                <p className="text-red-500 font-mono">${monteCarloResults.p10.toFixed(2)}</p>
              </div>
              <div className="bg-secondary/50 p-2 rounded">
                <p className="text-muted-foreground text-xs">25th %ile</p>
                <p className="text-orange-500 font-mono">${monteCarloResults.p25.toFixed(2)}</p>
              </div>
              <div className="bg-secondary/50 p-2 rounded">
                <p className="text-muted-foreground text-xs">Median</p>
                <p className="text-yellow-500 font-mono">${monteCarloResults.median.toFixed(2)}</p>
              </div>
              <div className="bg-purple-500/20 p-2 rounded border border-purple-500/50">
                ...
                <p className="text-purple-500 font-mono font-bold">${monteCarloResults.mean.toFixed(2)}</p>
              </div>
              <div className="bg-secondary/50 p-2 rounded">
                <p className="text-muted-foreground text-xs">75th %ile</p>
                <p className="text-lime-500 font-mono">${monteCarloResults.p75.toFixed(2)}</p>
              </div>
              <div className="bg-secondary/50 p-2 rounded">
                <p className="text-muted-foreground text-xs">90th %ile</p>
                <p className="text-emerald-500 font-mono">${monteCarloResults.p90.toFixed(2)}</p>
              </div>
              <div className="bg-secondary/50 p-2 rounded">
            ...
            <div className="h-48">
              <ResponsiveContainer width="100%" height="100%">
                <ComposedChart data={histogramData}>
                  <XAxis dataKey="price" stroke="hsl(var(--muted-foreground))" tick={{ fontSize: 10 }} />
                  ...
                  <ReferenceLine x={`$${currentPrice.toFixed(0)}`} stroke="#ef4444" strokeDasharray="3 3" label={{ value: 'Current', fill: '#ef4444', fontSize: 10 }} />
                </ComposedChart>
              </ResponsiveContainer>
            </div>
        ...
        <div className="h-64">
          <p className="text-sm text-muted-foreground mb-2">Projected Free Cash Flow (with growth decay)</p>
          <ResponsiveContainer width="100%" height="100%">
            <LineChart data={dcfResults.projections}>
              <XAxis dataKey="year" stroke="hsl(var(--muted-foreground))" />
              <YAxis stroke="hsl(var(--muted-foreground))" tickFormatter={(v) => formatNumber(v)} />
              <Tooltip
                contentStyle={{ backgroundColor: 'hsl(var(--card))', border: '1px solid hsl(var(--border))' }}
        ...
        {alternativeValuations.length > 0 && (
          <div className="bg-secondary/50 p-4 rounded-lg border border-border">
            <h3 className="text-lg font-bold mb-4">Alternative Valuation Methods</h3>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              {alternativeValuations.map((alt) => (
                <div key={alt.method} className="bg-secondary/50 p-3 rounded text-center">
                  ...
                  <p className={`text-xl font-bold font-mono ${alt.value > currentPrice ? 'text-emerald-500' : 'text-red-500'}`}>
                    ${alt.value.toFixed(2)}
                  </p>
        ...
        <div className="overflow-x-auto">
          <p className="text-sm text-muted-foreground mb-2">Sensitivity Analysis (Fair Value by WACC vs Terminal Growth)</p>
            ...
            <tbody>
              {[waccCalculation.wacc - 2, waccCalculation.wacc - 1, waccCalculation.wacc, waccCalculation.wacc + 1, waccCalculation.wacc + 2].map(w => (
                <tr key={w} className="border-b border-border/50">
                  <td className="p-2 font-medium">{w.toFixed(1)}%</td>
                  {[terminalGrowth - 1, terminalGrowth - 0.5, terminalGrowth, terminalGrowth + 0.5, terminalGrowth + 1].map(g => {
                    ...
                    return (
                      <td
                        key={`${w}-${g}`}
                        className={`p-2 text-center font-mono ${isHighlight ? 'bg-emerald-500/20 font-bold' : ''} ${fv > currentPrice ? 'text-emerald-500' : 'text-red-500'}`}
                      >
                        ${fv.toFixed(2)}
                      </td>
                    )
        ...
        <div className="text-xs text-muted-foreground/70 border-t border-border pt-4">
          ...
          <p className="mt-1"><strong className="text-foreground">Limitations:</strong> Model assumes predictable cash flows and perpetual growth. Terminal value typically represents 60-80% of total value, introducing significant uncertainty. Always combine with other valuation methods and qualitative analysis.</p>
        </div>
      </CardContent>
    </Card>
  )
