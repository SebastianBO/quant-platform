    ...
    <div className="space-y-6">
      ...
      <Card>
        ...
        <CardContent>
          ...
          <div className="mb-6">
            <p className="text-sm text-muted-foreground mb-2">52-Week Price Range</p>
          ...
          {technicals?.sharesShort && (
            <div className="p-4 bg-secondary/30 rounded-lg mb-6">
              <p className="font-medium mb-2">Short Interest Analysis</p>
      ...
      <Card>
        ...
        <CardContent>
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            {/* Rating Distribution */}
            <div>
              <p className="text-sm text-muted-foreground mb-3">Analyst Recommendation Distribution</p>
              <div className="h-48">
              ...
              <div className="text-center mt-2">
                <p className="text-muted-foreground text-sm">{analystRatings?.totalAnalysts} Analysts</p>
            ...
            {/* Price Target */}
            <div>
              <p className="text-sm text-muted-foreground mb-3">Price Target Analysis</p>
              <div className="space-y-4">
                <div className="p-4 bg-gradient-to-br from-emerald-500/20 to-emerald-500/5 rounded-lg border border-emerald-500/30 text-center">
                  <p className="text-muted-foreground text-sm">Wall Street Target Price</p>
                  <p className="text-3xl font-bold text-emerald-500">${targetPrice.toFixed(2)}</p>
                  <p className={`text-lg ${upside >= 0 ? 'text-emerald-500' : 'text-red-500'}`}>
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div className="p-3 bg-secondary/50 rounded-lg text-center">
                    ...
                    <p className={`font-bold ${upside >= 0 ? 'text-emerald-500' : 'text-red-500'}`}>
            ...
            <div className="space-y-6">
              ...
              <div>
                <div className="flex items-center justify-between mb-2">
                  <p className="font-medium">Piotroski F-Score</p>
                  <p className={`text-2xl font-bold ${
                ...
                <p className="text-sm text-muted-foreground mt-1">{qualityScores?.piotroskiInterpretation || 'Unknown'} Financial Health</p>
              </div>

              {/* Altman Z-Score */}
              <div>
                <div className="flex items-center justify-between mb-2">
                  <p className="font-medium">Altman Z-Score</p>
                  <p className={`text-2xl font-bold ${
          ...
          <CardContent>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
              <div className="p-3 bg-secondary/50 rounded-lg text-center">
                ...
                <p className="font-bold text-emerald-500">{formatCurrency(institutionalOwnership.totalValue)}</p>
              </div>
              <div className="p-3 bg-secondary/50 rounded-lg text-center">
                <p className="text-muted-foreground text-xs">Total Shares Held</p>
                <p className="font-bold">{(institutionalOwnership.totalShares / 1e6).toFixed(2)}M</p>
              </div>
              <div className="p-3 bg-secondary/50 rounded-lg text-center">
                <p className="text-muted-foreground text-xs">Number of Holders</p>
                <p className="font-bold">{institutionalOwnership.topHolders.length}+</p>
              </div>
              <div className="p-3 bg-secondary/50 rounded-lg text-center">
                <p className="text-muted-foreground text-xs">Avg Position</p>
                <p className="font-bold">{formatCurrency(institutionalOwnership.totalValue / institutionalOwnership.topHolders.length)}</p>
              </div>
            </div>
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-border">
                    ...
                    <th className="p-2 text-right">Report Date</th>
                  </tr>
                </thead>
                <tbody>
                  {institutionalOwnership.topHolders.slice(0, 10).map((holder: any, i: number) => (
                    <tr key={i} className="border-b border-border/50">
                      ...
                      <td className="p-2 text-right text-muted-foreground">{holder.report_period}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
...
function MetricCard({ label, value, highlight, positive }: { label: string; value: string | undefined; highlight?: boolean; positive?: boolean }) {
  return (
    <div className={`p-3 rounded-lg text-center ${highlight ? (positive ? 'bg-emerald-500/10 border border-emerald-500/30' : 'bg-red-500/10 border border-red-500/30') : 'bg-secondary/50'}`}>
      <p className="text-muted-foreground text-xs">{label}</p>
      <p className={`font-bold ${highlight ? (positive ? 'text-emerald-500' : 'text-red-500') : ''}`}>{value || 'N/A'}</p>
    </div>
  )
}
...
function SignalCard({ label, signal, positive, detail }: { label: string; signal: string; positive: boolean; detail: string }) {
  return (
    <div className={`p-4 rounded-lg ${positive ? 'bg-emerald-500/10 border border-emerald-500/30' : 'bg-red-500/10 border border-red-500/30'}`}>
  ...
  return (
    <div className="flex items-center justify-between p-3 bg-secondary/30 rounded-lg">
      <div>
        ...
        <p className={`text-sm ${color}`}>{interpretation}</p>
      </div>
