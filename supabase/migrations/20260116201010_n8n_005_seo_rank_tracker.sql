...
CREATE TABLE IF NOT EXISTS keyword_rankings (
    ...
    -- Position data
    ranking_position INTEGER, -- NULL if not in top 100
...
CREATE INDEX IF NOT EXISTS idx_keyword_rankings_position ON keyword_rankings(ranking_position) WHERE ranking_position IS NOT NULL;
...
CREATE TABLE IF NOT EXISTS seo_weekly_summaries (
    ...
    best_ranking_position INTEGER,
    worst_ranking_position INTEGER,
...
CREATE TABLE IF NOT EXISTS seo_keywords (
    ...
    -- Target metrics
    target_ranking_position INTEGER DEFAULT 10,
...
CREATE TABLE IF NOT EXISTS seo_alerts (
    ...
    -- Position data
    old_ranking_position INTEGER,
    new_ranking_position INTEGER,
...
-- Top performers (ranking_position 1-10)
...
SELECT *
FROM current_keyword_rankings
WHERE ranking_position IS NULL OR change_direction = 'down'
ORDER BY
    CASE WHEN ranking_position IS NULL THEN 1 ELSE 0 END,
    position_change ASC;
...
BEGIN
    ...
    SELECT
        COUNT(*),
        COUNT(*) FILTER (WHERE ranking_position IS NOT NULL),
        COUNT(*) FILTER (WHERE ranking_position <= 10),
        COUNT(*) FILTER (WHERE ranking_position <= 20),
