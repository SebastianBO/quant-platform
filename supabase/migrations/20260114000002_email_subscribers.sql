...
DROP TRIGGER IF EXISTS update_subscriber_timestamp ON email_subscribers;
...
DROP POLICY IF EXISTS "Service role full access to email log" ON email_send_log;
CREATE POLICY "Service role full access to email log" ON email_send_log
  FOR ALL USING (auth.role() = 'service_role');

DROP POLICY IF EXISTS "Service role full access to watchlist" ON email_watchlist;
...
DROP POLICY IF EXISTS "Users can manage own watchlist" ON email_watchlist;
